...
def create_title(titles, address, company, place):
    ...
    prompt = f"""
                     ...
                     3. 말투를 문장형 / 긴급형 / 정보형 / 후기형 등으로 조합해서 생성해 줘.
                        그리고 키워드 위치를 다양하게 변경해서 패턴의 획일화를 피해줘.       
    
                     4. ** 또는 ##와 같은 마크다운 언어는 쓰지 마.
                     제발. 마크다운 언어는 절대 포함하지 마. 어차피 적용 안돼
    
                     5. 그리고 너가 준 제목으로 바로 포스팅을 할거야. 다른 제목 옵션 주지 말고 그냥 제목 딱 한줄만 넘겨줘.
                     ...
                     7. 제목 글자 수는 25-40자 정도로 작성해 줘. 함축적으로 작성해주면 더 좋아.
                     
                     8. 아래 리스트는 지금까지 너가 생성해 준 제목 리스트야. 너가 지금 새로 생성할 제목과 비교했을 때, 문장 구조나 단어 배열 등 느낌이 겹치지 않게 생성해 줘.
                     특히, 앞 부분의 구조가 절대 중복되지 않도록 제대로 봐줘.
                     {title_list}.
    
                     지금까지 얘기한 8가지 요구사항들은 꼭 지켜줘. 하나도 빠짐없이 6개 다 지켜줘야 해.
    ...
    for i in range(5):
        try:
            ...
            log.append_log("token 수 = " + str(total_tokens))
            print("CLIENT 설정")
