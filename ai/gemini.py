...
def create_title(titles, address, company, place):
    ...
    try:
        response = model.generate_content(f"""
                    ...
                    5. 제목 글자 수는 50자 이내로 작성해 줘. 함축적으로 작성해주면 더 좋아. 100자가 넘어가면 제목 입력이 되지 않아.
                    
                    6. 중요한 사항이 있어. 인공지능이 썼단 느낌보다 사람이 수기로 생각해서 썼다고 생각하도록 잘 다듬어서 써 줘.
                    
                    7. 아래 리스트는 지금까지 너가 생성해 준 제목 리스트야. 너가 지금 새로 생성할 제목과 비교했을 때, 문장 구조나 단어 배열 등 느낌이 겹치지 않게 생성해 줘.
                    주소와 업종만 다르다고 다른 제목이 아니야. 다른 사람이 봤을 때 "새로운" 제목으로 느껴지게 작성해야 해. 
                    {title_list}
